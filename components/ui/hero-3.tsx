...
const ActionButton = ({ children, href }: { children: React.ReactNode; href?: string }) => {
  const btnClass = "mt-8 inline-block px-8 py-3 rounded-full bg-indigo-500 text-white font-semibold shadow-lg transition-colors hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-opacity-75";
  if (href) {
    return (
      <Link href={href} legacyBehavior passHref>
        <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className={btnClass}>
          {children}
        </motion.a>
      </Link>
    );
